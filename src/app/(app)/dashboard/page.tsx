...
function calcBestConditions(data: DashData) {
  ...
  const avgCold = highSets.filter(s => s.coldBathSeconds != null).reduce((sum, s, _, arr) =>
    sum + s.coldBathSeconds! / arr.length, 0)

  const highConds = data.conditions.filter(c => high.some(h => h.id === c.sessionId))
  const avgSleep = highConds.filter(c => c.sleepHours != null).reduce((sum, c, _, arr) =>
    sum + c.sleepHours! / arr.length, 0)

  const loylyRate = Math.round(high.filter(s => highSets.filter(x => x.sessionId === s.id).some(x => x.loyly)).length / high.length * 100)
  const outdoorRate = Math.round(high.filter(s => highSets.filter(x => x.sessionId === s.id).some(x => x.restType === 'outdoor')).length / high.length * 100)
    ...
    <div className="px-4 pt-6 pb-4">
      ...
      {loading && (
        <div className="space-y-4">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="sauna-card animate-pulse h-40" />
          ))}
        </div>
      )}

      {!loading && data && data.sessions.length === 0 && (
        <div className="sauna-card text-center py-10 text-gray-500">
          <p className="text-sm">記録が3件以上になると詳しい分析が表示されます</p>
        </div>
      )}
          ...
          {best && (
            <div className="bg-gradient-to-br from-[#2A1A0A] to-[#1E1E1E] border border-[#D4853A]/30 rounded-2xl p-4 mb-5">
              ...
              <p className="text-sm text-gray-200 leading-relaxed">
                ...
                <br />
                {best.avgCold > 0 && <><span className="text-white font-semibold">水風呂 {Math.round(best.avgCold)}秒前後</span>、</>}
                {best.avgSleep > 0 && <><span className="text-white font-semibold">睡眠 {best.avgSleep.toFixed(1)}h以上</span></>}
